...
export function sanitizeStakeholderCodeValue(value: unknown): string | null {
...
export function sanitizeStakeholderUrl(value: unknown): string | null {
  const trimmed = trimToNull(value)
  if (!trimmed || isPlaceholderToken(trimmed)) return null

  const parsed = parseStakeholderUrl(trimmed)
  if (!parsed) return null
  if (parsed.origin !== 'https://localvip.invalid' && !['http:', 'https:'].includes(parsed.protocol)) {
    return null
  }
