...
// QA-only causes have no Supabase row, so resolve their public landing page
// straight from the QA PublicJoin endpoint. Capture stays decoupled from any
// referral code — joining a supporter list never attaches one.
async function resolveQaCauseName(slug: string): Promise<string | null> {
  try {
    ...
    if (json?.kind === 'cause' && json.entity?.name) return json.entity.name
    return null
  } catch {
    return null
  }
...
export default async function CommunitySupportPage({
  ...
  if (!cause) {
    const qaCauseName = await resolveQaCauseName(params.slug)
    if (!qaCauseName) {
      notFound()
    }
            ...
            <div className="rounded-[2rem] border border-white/70 bg-white/92 p-5 shadow-xl sm:p-6">
              <div className="rounded-[1.75rem] bg-gradient-to-br from-pink-600 via-pink-500 to-rose-500 p-5 text-white shadow-lg">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <h1 className="mt-2 text-3xl font-black tracking-tight text-white">{qaCauseName}</h1>
                ...
                <div className="mt-5 rounded-[1.5rem] border border-white/15 bg-white/10 p-4">
                  ...
                  <p className="mt-2 text-2xl font-bold leading-tight text-white">Join the {qaCauseName} supporter list</p>
                  <p className="mt-3 text-sm leading-6 text-white/85">Add your details below to support local causes and businesses near you.</p>
                </div>
              </div>
              ...
              <div className="mt-5">
                <PublicCommunitySupportForm
                  slug={params.slug}
                  causeName={qaCauseName}
                  headline={`Join the ${qaCauseName} supporter list`}
                />
              </div>
            </div>
          </div>
