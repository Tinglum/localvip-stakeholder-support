...
export async function POST(
  ...
  // QA path: params.id is the numeric QA nonprofit id
  if (session.source === 'qa' && /^\d+$/.test(params.id)) {
    ...
    try {
      ...
      const res = await fetchQaApi(`/api/dashboard/v1/Nonprofit/${encodeURIComponent(params.id)}/${endpoint}`, {
        method: 'POST',
        body: fd,
      })
  ...
  try {
    ...
    const filePath = `cause-media/${params.id}/${mediaType}.${ext}`
    ...
    if (uploadError) {
      console.error('[cause-media] storage upload error', uploadError)
    ...
    const patch: Partial<Cause> = mediaType === 'logo'
      ? { logo_url: publicUrl }
    ...
    if (updateError) {
      console.error('[cause-media] causes update error', updateError)
      return NextResponse.json(
        {
          error: `Cause update failed: ${updateError.message || 'unknown error'}`,
