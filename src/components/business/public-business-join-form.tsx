...
export function PublicBusinessJoinForm({
  ...
  const [wantsBusinessOffers, setWantsBusinessOffers] = React.useState(true)
    ...
    return (
      <div className="rounded-[2rem] border border-emerald-200 bg-white/96 p-6 shadow-[0_24px_80px_-35px_rgba(16,185,129,0.55)] backdrop-blur">
        ...
        <div className="mt-5 rounded-[1.5rem] border border-surface-200 bg-surface-50 p-4 text-center">
          <p className="text-sm font-semibold text-surface-900">You&apos;re all set.</p>
          <p className="mt-2 text-sm leading-6 text-surface-600">
            Show this screen at checkout to claim your offer. {supportLabel}.
          </p>
        </div>
      </div>
    )
      ...
      <form onSubmit={handleSubmit} className="mt-6 space-y-4">
        ...
        <div>
          ...
          <Input
            ...
            required
          />
          <p className="mt-1 text-xs text-surface-500">Both phone and email are required so the business can confirm your offer.</p>
        </div>
        ...
        <label className="flex items-start gap-3 rounded-2xl border border-surface-200 bg-surface-50 px-4 py-3">
          <input
            type="checkbox"
            checked={wantsBusinessOffers}
            onChange={(event) => setWantsBusinessOffers(event.target.checked)}
            className="mt-1 h-4 w-4 rounded border-surface-300 text-brand-600 focus:ring-brand-500"
          />
          <span className="text-sm leading-6 text-surface-700">
            I want to hear from {businessName} when they have good offers that benefit me.
          </span>
        </label>
